...
- Schema migrations 5–8 replace the three `journal_event_*_links` tables with
  direct foreign keys, add a required `command_queue_items.instance_id`, and
  widen balance/limit columns to `bigint`. They also move command insertion and
  queue-item insertion/update timestamps to the PostgreSQL clock. Upgrades from
  0.4.x must use `DoubleEntryLedger.Migration.up(from: 4)`.
...
- `commands.inserted_at`, `command_queue_items.inserted_at`, and
  `command_queue_items.updated_at` are generated by PostgreSQL and read back
  after writes, avoiding application-node clock skew.
