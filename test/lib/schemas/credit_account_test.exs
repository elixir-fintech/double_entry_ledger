defmodule DoubleEntryLedger.CreditAccountTest do
  ...
  describe "Credit Account update balances [:posted]: " do
    ...
    test "first debit entry", %{instance: inst} do
      account = account_fixture(type: :credit, instance_id: inst.id)
      entry = %Entry{type: :debit, amount: Money.new(200, :EUR) }
    ...
    test "debit entry with previous balance", %{instance: inst} do
      account = account_fixture(type: :credit, instance_id: inst.id, posted: %{amount: -100, debit: 100, credit: 0}, available: 0)
      entry = %Entry{type: :debit, amount: Money.new(200, :EUR) }
      assert %Ecto.Changeset{
        valid?: true,
        changes: %{
    ...
    test "first credit entry", %{instance: inst} do
      account = account_fixture(type: :credit, instance_id: inst.id)
      entry = %Entry{type: :credit, amount: Money.new(200, :EUR) }
      assert %Ecto.Changeset{
        valid?: true,
        changes: %{
          available: 200,
    ...
    test "credit entry with previous balance", %{instance: inst} do
      account = account_fixture(type: :credit, instance_id: inst.id, posted: %{amount: 100, debit: 0, credit: 100}, available: 100)
      entry = %Entry{type: :credit, amount: Money.new(50, :EUR) }
      assert %Ecto.Changeset{
        valid?: true,
  ...
  describe "Credit Account update balances [:pending]: " do
    ...
    test "debit entry with previous balance", %{instance: inst} do
      ...
      entry = %Entry{type: :debit, amount: Money.new(25, :EUR) }
      assert %Ecto.Changeset{
        valid?: true,
        changes: %{
          available: 25,
    ...
    test "credit entry with previous balance", %{instance: inst} do
      ...
      entry = %Entry{type: :credit, amount: Money.new(25, :EUR) }
      assert %Ecto.Changeset{
        valid?: true,
        changes: %{
          available: 100,
