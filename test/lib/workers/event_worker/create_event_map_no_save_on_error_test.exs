defmodule DoubleEntryLedger.EventWorker.CreateEventMapNoSaveOnErrorTest do
  @moduledoc """
  ...
  describe "process/1" do
    ...
    test "return EventMap changeset for duplicate source_idempk", ctx do
      ...
      {:error, changeset} = CreateEventMapNoSaveOnError.process(event_map)
  ...
  describe "process_map/2 with OCC timeout" do
    ...
    test "with last retry that fails", ctx do
      ...
      assert {:error, %Changeset{data: %EventMapSchema{}, errors: [occ_timeout: _]}} =
               CreateEventMapNoSaveOnError.process(
                 struct(
                   EventMapSchema,
                   event_map(ctx)
                 ),
