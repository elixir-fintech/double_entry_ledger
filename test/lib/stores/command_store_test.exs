defmodule DoubleEntryLedger.Stores.CommandStoreTest do
  ...
  describe "create/1" do
    ...
    test "reads command and queue insertion timestamps generated by the database", %{
      instance: instance
    } do
      assert {:ok, command} =
               CommandStore.create(transaction_command_attrs(instance_address: instance.address))

      assert %DateTime{} = command.inserted_at
      assert %DateTime{} = command.command_queue_item.inserted_at
      assert %DateTime{} = command.command_queue_item.updated_at
    end

    test "assigns increasing database-generated queue positions", %{instance: instance} do
      assert {:ok, first} =
               CommandStore.create(
                 transaction_command_attrs(
                   instance_address: instance.address,
                   source_idempk: "first-position"
                 )
               )
      ...
      assert {:ok, second} =
               CommandStore.create(
                 transaction_command_attrs(
                   instance_address: instance.address,
                   source_idempk: "second-position"
                 )
               )

      assert is_integer(first.command_queue_item.queue_position)
      assert second.command_queue_item.queue_position > first.command_queue_item.queue_position
    end
