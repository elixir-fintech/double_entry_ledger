defmodule DoubleEntryLedger.EventWorker.UpdateEvent do
  ...
  defp fetch_create_event_transaction(%{
         id: e_id,
         source: source,
  ...
  defp update_transaction_and_event(event, transaction, attrs) do
    case build_update_transaction_and_event(transaction, event, attrs)
         |> Repo.transaction() do
  ...
  @spec retry_update_event(Event.t(), Transaction.t(), map(), integer()) ::
          {:ok, {Transaction.t(), Event.t()}} | {:error, String.t()}
  defp retry_update_event(event, transaction, transaction_map, attempts) when attempts > 0 do
    try do
  ...
  defp retry_update_event(_event, _transaction, _transaction_map, 0) do
