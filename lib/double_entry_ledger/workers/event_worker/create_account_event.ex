    ...
    |> case do
      ...
      {:error, :account, changeset, _changes} ->
        Logger.warning(
          ...
          Event.log_trace(event, changeset.errors)
        )

        {:ok, dead_event} = build_mark_as_dead_letter(event, "Account changeset invalid") |> Repo.update()
      ...
      {:error, step, error, _steps_so_far} ->
        ...
        Logger.error(
          "#{@module_name}: Step :#{step} failed.",
          Event.log_trace(event, error)
        )

        {:error, :multi_step_failed}
    end
  end

  @spec build_create_account(Event.t()) :: Ecto.Multi.t()
  defp build_create_account(
         %Event{event_map: event_map, instance_id: instance_id} = event
       ) do

    account_data = %AccountEventMap{}
